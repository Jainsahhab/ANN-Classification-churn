...
#LOad  the trained mmodel

model=tf.keras.models.load_model('model.h5')


##load all the ANN trained model ,scaler pickle file ,one hot encoding


##load the encoderans scaler
...
# 1‑hot encode the Geography value
geo_encoded = onehot_encoder_geo.transform([[geography]]).toarray()

# Build a DataFrame using the encoder’s own feature names
geo_encoded_df = pd.DataFrame(
    geo_encoded,
    columns=onehot_encoder_geo.get_feature_names_out()
)

# Combine the one‑hot columns with the rest of your inputs
